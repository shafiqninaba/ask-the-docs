...
from streamlit_app.src.utils.login import authenticate

load_dotenv()

st.set_page_config(page_title="Firecrawl App", page_icon="🔥", layout="centered")

FASTAPI_BACKEND = os.getenv("FASTAPI_BACKEND")

# replace http or https with ws or wss
WEBSOCKET_URL = urljoin(
    FASTAPI_BACKEND.replace("http", "ws").replace("https", "wss"), "firecrawl/ws/crawl"
)
...
if st.session_state.authenticated:
    ...
    limit = st.slider(
        "Maximum pages to crawl", min_value=10, max_value=1000, value=500, step=10
    )
                ...
                async with websockets.connect(WEBSOCKET_URL) as websocket:
                    ...
                    try:
                        ...
                        if st.session_state["firecrawler_messages"] and "done" not in [
                            ...
                            st.session_state["is_crawling"] = (
                                False  # Mark crawling as complete
                            )
                            st.session_state["start_time"] = None
                            button_container.button(
                                "Start Crawl", key="restart_crawl_button"
                            )  # Added unique key
